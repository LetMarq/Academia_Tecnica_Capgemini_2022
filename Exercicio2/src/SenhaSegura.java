...
public class SenhaSegura {
	...
	public static void main(String[] args) {
				...
				if(!uppercase) password_last++;
				if(!lowercase) password_last++;
				if(!special_char) password_last++;
				if(!digit) password_last++;				
				if(pass_len < 6 && (6-pass_len) > password_last ) {
					password_last = 6 - pass_len ;
				}
				System.out.println("O número de caracteres mínimos que faltam é: "+ password_last);
			}
